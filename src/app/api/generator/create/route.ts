  ...
  try {
    ...
    return Response.json({
      ...
      nextSteps: [
        ...
        `운영 Vercel 프로젝트에 아래 환경변수 ${deploymentEnv.length}개를 모두 입력합니다.`,
        '배포 완료 후 /, /bank, /admin/login 주소가 열리고 시트 데이터가 표시되는지 확인합니다.',
      ],
      deploymentGuide: buildDeploymentGuide(result.spreadsheetId, deploymentEnv),
    });
  } catch (error) {
...
function buildDeploymentGuide(spreadsheetId: string, deploymentEnv: Array<{ name: string }>) {
  const templateRepositoryUrl = process.env.NEXT_PUBLIC_CLASS_STORE_TEMPLATE_REPO?.trim();
  const envNames = deploymentEnv.map((env) => env.name);
  const envDescription = `학급 보상 시스템 운영에 필요한 환경변수입니다. ${envNames.length}개 값을 모두 입력해야 배포된 앱이 Google Sheets를 읽고 쓸 수 있습니다.`;
  ...
  return {
    ...
    checklist: [
      ...
      'CLASS_STORE_STORAGE=sheets를 입력해 Google Sheets 저장소를 사용합니다.',
