...
export async function POST(request: Request) {
  ...
  try {
    ...
    return grantResponse({
      ...
      deploymentGuide: buildDeploymentGuide(result.spreadsheetId, deploymentEnv.map((item) => item.name)),
    });
  } catch (error) {
    if (grant) {
...
function buildDeploymentGuide(spreadsheetId: string, envNames: string[]) {
  ...
  return {
    ...
    checklist: [
      ...
      'CLASS_STORE_STORAGE에 sheets를 정확히 입력합니다. 저장소 종류는 자동 선택되지 않습니다.',
