...
import { LATEST_SCHEMA_VERSION, SYSTEM_VERSION } from '@/generator/config/versions';
...
describe('settings', () => {
  ...
  it('uses env spreadsheet id and default currency unit when Settings sheet is unavailable', async () => {
    ...
    expect(settings).toEqual({ spreadsheetId: 'env-sheet-id', currencyUnit: '원', appTitle: '학급 매점', bankTitle: '학급 은행', themeColor: 'white', schemaVersion: LATEST_SCHEMA_VERSION, systemVersion: SYSTEM_VERSION, systemName: '학급 보상 시스템', source: 'env' });
  });
  ...
  it('saves currency unit and app title to Settings sheet and rejects changing deployment spreadsheet id', async () => {
    ...
    ).resolves.toEqual({ spreadsheetId: 'env-sheet-id', currencyUnit: '달란트', appTitle: '햇살반 매점', bankTitle: '햇살반 은행', themeColor: 'green', schemaVersion: LATEST_SCHEMA_VERSION, systemVersion: SYSTEM_VERSION, systemName: '학급 보상 시스템', source: 'sheet' });
    ...
    expect(appends).toEqual([
      ...
      { sheetName: 'Settings', values: ['schemaVersion', String(LATEST_SCHEMA_VERSION)] },
