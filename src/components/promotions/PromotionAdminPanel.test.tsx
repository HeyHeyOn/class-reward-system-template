...
describe('PromotionAdminPanel', () => {
  it('keeps a created promotion when the initial GET resolves stale after the POST', async () => {
    ...
    expect(screen.getByTestId('promotion-row').textContent).toContain('PROMO-NEW');
  });

  it('keeps an edited promotion when an earlier refresh GET resolves stale after the PATCH', async () => {
    ...
    fireEvent.change(screen.getByLabelText('행사명'), { target: { value: '수정 할인' } });
    fireEvent.change(screen.getByLabelText('할인율'), { target: { value: '15' } });
    fireEvent.click(screen.getByRole('button', { name: '행사 수정 저장' }));
  ...
  it('shows loading, canonical-order list details, empty state, and safe retry errors', async () => {
    ...
    expect(rows[1].textContent).toContain('N+1');
    expect(rows[1].textContent).toContain('연필');
    expect(rows[1].textContent).toContain('2개 구매 + 1개 무료');
    fireEvent.click(screen.getByRole('button', { name: '행사 목록 새로고침' }));
  ...
  it('edits with a full exact payload and toggles activation with activation-only PATCH', async () => {
    ...
    await screen.findByText('기존 10% 할인');
    fireEvent.click(screen.getByRole('button', { name: '기존 10% 할인 편집' }));
    expect(screen.getByLabelText('시작 일시')).toHaveProperty('value', '2026-08-01T09:00');
    expect(screen.getByLabelText('종료 일시')).toHaveProperty('value', '2026-09-01T09:00');
    fireEvent.change(screen.getByLabelText('행사명'), { target: { value: '수정 할인' } });
    fireEvent.change(screen.getByLabelText('할인율'), { target: { value: '15' } });
    fireEvent.click(screen.getByRole('button', { name: '행사 수정 저장' }));
  ...
  it('does not append or replace list state when a successful save response is malformed', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '기존 10% 할인 편집' }));
    fireEvent.change(screen.getByLabelText('행사명'), { target: { value: '수정 시도' } });
    fireEvent.click(screen.getByRole('button', { name: '행사 수정 저장' }));
    expect((await screen.findByRole('alert')).textContent).toContain('행사 응답 형식이 올바르지 않습니다.');
  ...
  it('reorders immediately after a low-priority create and a priority edit', async () => {
    ...
    fireEvent.change(screen.getByLabelText('정렬 우선순위'), { target: { value: '-1' } });
    fireEvent.click(screen.getByRole('button', { name: '행사 수정 저장' }));
    await screen.findByText('행사를 수정했습니다.');
    expect(screen.getAllByTestId('promotion-row').map((row) => row.textContent)).toEqual([
  ...
  it('locks every draft and session-changing control while a save is pending', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '기존 10% 할인 편집' }));
    fireEvent.change(screen.getByLabelText('행사명'), { target: { value: '저장 중인 A' } });
    fireEvent.click(screen.getByRole('button', { name: '행사 수정 저장' }));
    await screen.findByRole('status', { name: '행사 저장 중' });

    const editB = screen.getByRole('button', { name: '연필 2+1 편집' }) as HTMLButtonElement;
    const cancel = screen.getByRole('button', { name: '편집 취소' }) as HTMLButtonElement;
    ...
    expect(screen.getByRole('heading', { name: '행사 편집' })).toBeTruthy();
    expect(screen.getByLabelText('행사명')).toHaveProperty('value', '저장 중인 A');
